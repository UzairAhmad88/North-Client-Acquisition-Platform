...
import { queryDataCopilot, DataCopilotResponse } from "@/lib/api/enterpriseDataOs";

export const DataCopilotView: React.FC = () => {
  const [query, setQuery] = useState("");
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<DataCopilotResponse | null>(null);
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await queryDataCopilot(query);
      setResult(res);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const sampleQueries = [
    "Where does ARR come from and what is its semantic definition?",
    "Show lineage and blast radius for silver customer profiles dataset.",
    "What is the overall data quality index across Bronze and Silver tiers?",
    "Find optimization opportunities to reduce Lakehouse storage spend.",
  ];

  return (
    <div className="bg-slate-900/60 backdrop-blur border border-slate-800 rounded-xl p-5 shadow-sm">
      <div className="pb-4 border-b border-slate-800">
        <h2 className="text-lg font-semibold text-slate-100">Enterprise Data Copilot</h2>
        <p className="text-xs text-slate-400">
          Grounded semantic data intelligence distinguishing FACT, INFERENCE, HYPOTHESIS, and RECOMMENDATION
        </p>
      </div>
      ...
      <form onSubmit={handleAsk} className="mt-4 flex gap-2">
        <input
          ...
          placeholder="Ask Data Copilot about lineage, metrics, contracts, datasets, quality, or costs..."
      ...
      {result && (
        <div className="mt-5 space-y-3 bg-slate-800/30 p-4 rounded-xl border border-slate-700/60 text-xs">
          <div className="flex items-center justify-between pb-2 border-b border-slate-700/50">
            <span className="font-semibold text-slate-200">Grounded Semantic Intelligence</span>
